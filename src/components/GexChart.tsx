...
// ─── Palette ────────────────────────────────────────────────────────────────
const POS   = "#1de9b6";  // bright teal – positive / Call GEX bars
const NEG   = "#f87171";  // coral/salmon – negative / Put GEX bars
const C_SPOT = "#eab308"; // yellow  – Spot
const C_CW   = "#ef4444"; // red     – Call Wall
const C_PW   = "#22c55e"; // green   – Put Wall
const C_FLIP = "#818cf8"; // indigo  – Gamma Flip
const C_NET  = "#60a5fa"; // blue    – Net line in Call vs Put mode
// ────────────────────────────────────────────────────────────────────────────
...
export default function GexChart({ strikes, summary, spot, priceLabel = "" }: Props) {
  ...
  // For categorical XAxis, reference lines must use exact strike values
  const xSpot  = nearestTo(strikes, spot);
  const xFlip  = summary.gammaFlip != null ? nearestTo(strikes, summary.gammaFlip) : null;
  // callWall & putWall are already exact strikes from the calculator
    ...
    <div className="bg-[#0d1117] border border-[#30363d] rounded-xl p-4">
      ...
      <div className="flex items-center justify-end flex-wrap gap-x-3 gap-y-1 mb-1">
        {mode === "net" ? (
          ...
          <>
            <LegendBar color={POS} label="Call GEX" />
            <LegendBar color={NEG} label="Put GEX" />
          </>
        )}
      ...
      <ResponsiveContainer width="100%" height={340}>
        <ComposedChart data={strikes} margin={{ top: 20, right: 8, left: 8, bottom: 44 }}>
          ...
          {/* Range shading between PW and CW */}
          {xPW && xCW && (
            <ReferenceArea
              x1={Math.min(xPW, xCW)}
              x2={Math.max(xPW, xCW)}
          ...
          {/* Gamma Flip – label at bottom to avoid crowding the top */}
          {xFlip && (
            <ReferenceLine
              x={xFlip}
              stroke={C_FLIP}
          ...
          {mode === "callput" ? (
            ...
            <Bar dataKey="netGex" name="Net GEX" radius={[2, 2, 2, 2]}>
              {strikes.map((s, i) => (
                <Cell key={i} fill={s.netGex >= 0 ? POS : NEG} />
              ))}
            </Bar>
          )}
        </ComposedChart>
